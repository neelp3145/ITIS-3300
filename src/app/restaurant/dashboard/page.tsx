...
interface KitchenStats {
  ...
  popularItems: Array<{ _id: string; totalQuantity: number; totalRevenue: number }>;
}
...
const mockOrders: Order[] = [
  {
    ...
    status: 'preparing',
    items: [
      { name: 'Classic Burger', quantity: 2, price: 12.99, specialInstructions: 'No onions' },
...
const mockStats: KitchenStats = {
  ...
  todayOrders: [{ count: 12, revenue: 285.50 }],
  popularItems: [
...
const CustomAlert = ({ status, children }: { status: 'error' | 'warning' | 'info'; children: React.ReactNode }) => {
  const colors = {
    error: { bg: 'red.50', border: 'red.200', text: 'red.800', emoji: '❌' },
    warning: { bg: 'yellow.50', border: 'yellow.200', text: 'yellow.800', emoji: '⚠️' },
...
const LiveTimer = ({ createdAt, estimatedPrepTime, status }: {
  createdAt: string;
  estimatedPrepTime: number;
  status: string;
}) => {
...
const OrderCard = ({
  ...
  const [assignedDrivers, setAssignedDrivers] = useState<{ [key: string]: string }>({});
    ...
    <div style={{
      ...
      <div style={{ padding: '1.5rem' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {/* Order Items */}
          <div>
            <span style={{ fontWeight: 'bold', fontSize: '0.875rem', marginBottom: '0.75rem', display: 'block', color: '#374151' }}>
              📋 Order Items:
            </span>
            {order.items.map((item, index) => (
              <div key={index} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.875rem', marginBottom: '0.5rem', padding: '0.25rem 0' }}>
                <div>
          ...
          {showKitchenNotes && (
            <div>
              <span style={{ fontSize: '0.75rem', fontWeight: 'bold', marginBottom: '0.5rem', display: 'block', color: '#374151' }}>
                📝 Kitchen Notes:
              </span>
              <textarea
                style={{
                ...
                onChange={(e) => setKitchenNotes(prev => ({
                  ...prev,
...
export default function RestaurantDashboard() {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      setKitchenOrders(mockOrders.filter(order => ['pending', 'confirmed', 'preparing'].includes(order.status)));
      setDeliveryOrders(mockOrders.filter(order => order.orderType === 'delivery' && order.status === 'ready'));
      setDineInOrders(mockOrders.filter(order => order.orderType === 'dine-in' && ['ready', 'preparing'].includes(order.status)));
      setStats(mockStats);
    } catch (err) {
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {orders.length === 0 && (
              <div style={{ textAlign: 'center', padding: '2rem' }}>
                <span style={{ color: '#6b7280', fontSize: '1.125rem' }}>No orders found</span>
              </div>
            )}
          </div>
        );
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {kitchenOrders.length === 0 && (
              <div style={{ textAlign: 'center', padding: '2rem' }}>
                <span style={{ color: '#6b7280', fontSize: '1.125rem' }}>No orders in kitchen queue</span>
              </div>
            )}
          </div>
        );
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {deliveryOrders.length === 0 && (
              <div style={{ textAlign: 'center', padding: '2rem' }}>
                <span style={{ color: '#6b7280', fontSize: '1.125rem' }}>No orders ready for delivery</span>
              </div>
            )}
          </div>
        );
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            {dineInOrders.length === 0 && (
              <div style={{ textAlign: 'center', padding: '2rem' }}>
                <span style={{ color: '#6b7280', fontSize: '1.125rem' }}>No active dine-in orders</span>
              </div>
            )}
          </div>
        );
      ...
      <div style={{ padding: '1.5rem', backgroundColor: '#f9fafb', minHeight: '100vh' }}>
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '50vh' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
            <div style={{ width: '3rem', height: '3rem', border: '4px solid #ea580c', borderTop: '4px solid transparent', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div>
    ...
    <div style={{ padding: '1.5rem', backgroundColor: '#f9fafb', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: '0.25rem' }}>
            <h1 style={{ fontSize: '2.25rem', fontWeight: 'bold', color: '#ea580c' }}>FastBite Restaurant Dashboard</h1>
            <span style={{ color: '#6b7280' }}>Manage orders, kitchen queue, and deliveries</span>
          </div>

          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
            {showClearAllConfirm ? (
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', backgroundColor: '#fef2f2', padding: '0.5rem', borderRadius: '0.375rem', border: '1px solid #fecaca' }}>
                <span style={{ fontSize: '0.875rem', color: '#dc2626', fontWeight: 'bold' }}>Clear ALL orders?</span>
                <button
                  style={{
                    fontSize: '0.75rem',
        ...
        <div style={{
          ...
          <StatCard
            label="In Progress"
            value={stats?.statusCounts.find(s => s._id === 'preparing')?.count || 0}
            helpText="Being prepared"
            color="#3b82f6"
          />
          <StatCard
            label="Ready"
            value={stats?.statusCounts.find(s => s._id === 'ready')?.count || 0}
